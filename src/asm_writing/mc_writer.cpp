...
class X86MCWriter : public MCWriter {
    private:
        ...
        /// "add $val, %reg"
        inline void _emitAdd(int reg, int val) {
            _emitArith(reg, val, X86::OPCODE_ADD);
        }

        /// "sub %val, %reg"
        inline void _emitSub(int reg, int val) {
            _emitArith(reg, val, X86::OPCODE_SUB);
        }
    ...
    public:
        ...
        virtual int numArgRegs() {
            return X86::NUM_ARG_REGS;
        }

        virtual int numTempRegs() {
            return 2;
        }

        virtual void emitNop() {
            _emitByte(0x90);
        }

        virtual void emitTrap() {
            _emitByte(0xcc);
        }
        ...
        virtual void emitGetattr(int src_argnum, int src_offset,
                int dest_argnum) {
            int srcreg = convertArgnum(src_argnum);
            int destreg = convertArgnum(dest_argnum);
        ...
        virtual void emitSetattr(int src_argnum, int dest_argnum,
                int dest_offset) {
            int srcreg = convertArgnum(src_argnum);
            int destreg = convertArgnum(dest_argnum);
        ...
        virtual void emitPush(int argnum) {
            _emitPush(convertArgnum(argnum));
        }

        virtual void emitPop(int argnum) {
            _emitPop(convertArgnum(argnum));
        }
