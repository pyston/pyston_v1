...
public:
    ...
    void accept(gc::GCVisitor* v) {
        ...
        if (pub_state->curexc_type)
            v->visit((void**)&pub_state->curexc_type);
        if (pub_state->curexc_value)
            v->visit((void**)&pub_state->curexc_value);
        if (pub_state->curexc_traceback)
            v->visit((void**)&pub_state->curexc_traceback);
        if (pub_state->dict)
            v->visit((void**)&pub_state->dict);

        for (auto& stack_info : previous_stacks) {
            v->visit((void**)&stack_info.next_generator);
#if STACK_GROWS_DOWN
