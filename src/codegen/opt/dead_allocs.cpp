...
class DeadAllocsPass : public FunctionPass {
    private:
        ...
        Value* deriveSimilarly(Value* derived, Value* ancestor, Value* new_ancestor, Instruction *insert_before, std::vector<Instruction*> &added) {
            if (derived == ancestor)
                return new_ancestor;
        ...
        Value* extractLoadValue(Value* ptr, Instruction* inst, ChainInfo &chain) {
            ...
            if (StoreInst *si = dyn_cast<StoreInst>(inst)) {
                ...
                if (ar == AliasAnalysis::MustAlias) {
                    ...
                    if (dl->getTypeStoreSize(elt_type) == dl->getTypeStoreSize(si->getValueOperand()->getType())) {
                        Instruction::CastOps cast_opcode = CastInst::getCastOpcode(si->getValueOperand(), true, elt_type, true);
            ...
            if (PHINode* phi = dyn_cast<PHINode>(inst)) {
                ...
                if (VERBOSITY() >= 2) errs() << "Derived from phi: " << *phi << "; pushing back and adding " << *load_phi << "\n";
                for (int i = 0, e = phi->getNumIncomingValues(); i < e; i++) {
                    ...
                    if (VERBOSITY() >= 2) errs() << "Phi-recursing on " << *prev_derived << '\n';
                    ...
                    if (prev_resolved == NULL) {
                        if (VERBOSITY() >= 2) errs() << "Wasn't able to resolve " << *prev_derived << "; just emitting a load\n";
                        prev_resolved = new LoadInst(prev_derived, "t", prev_bb->getTerminator());
                    } else {
                        if (VERBOSITY() >= 2) errs() << "Resolved " << *prev_derived << " to " << *prev_resolved << "; deleting the temporary instructions\n";
                        for (int i = added.size() - 1; i >= 0; i--) {
                            if (VERBOSITY() >= 2) errs() << "Deleting temporary " << *added[i] << '\n';
        ...
        Value* getLoadValFromPrevious(Value* ptr, BasicBlock* bb, std::unordered_map<BasicBlock*, Value*> &seen, ChainInfo &chain) {
            Value* &r = seen[bb];
            if (r != NULL)
                return r;
            ...
            PHINode* phi = PHINode::Create(cast<PointerType>(ptr->getType())->getElementType(), bb->getNumUses(), "t", bb->getFirstNonPHI());
            r = phi;
            ...
            for (auto prev_bb = pred_begin(bb), end = pred_end(bb); prev_bb != end; ++prev_bb) {
                ...
                if (VERBOSITY() >= 2) errs() << "Recursing into " << (*prev_bb)->getName() << '\n';
                Value* v = getLoadValFrom(ptr, *prev_bb, seen, chain);
                if (VERBOSITY() >= 2) errs() << "Done recursing into " << (*prev_bb)->getName() << '\n';
            ...
            if (VERBOSITY("opt") >= 1) errs() << "Finished adding phi in " << bb->getName() << ": " << *phi << '\n';
            return phi;
        }
        ...
        void remapLoad(LoadInst* li, ChainInfo &chain) {
            ...
            while (it != li->getParent()->rend()) {
                ...
                assert(new_v);
                if (VERBOSITY("opt") >= 1) errs() << "Remapped to: " << *new_v << '\n';
