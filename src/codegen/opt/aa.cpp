...
class PystonAA : public ImmutablePass, public AliasAnalysis {
    ...
    public:
        ...
        PystonAA() : ImmutablePass(ID), depth(0) {
            initializePystonAAPass(*PassRegistry::getPassRegistry());
        }
        ...
        AliasResult _alias(const Location &LocA, const Location &LocB) {
            ...
            if (VERBOSITY("opt.aa") >= 2) {
                ...
                indent(); errs() << LocA.Size << "  "; LocA.Ptr->dump();
                indent(); errs() << LocB.Size << "  "; LocB.Ptr->dump();
                indent(); errs() << "base: " << base << '\n';
            }
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (VERBOSITY("opt.aa") >= 2) {
                    indent(); errs() << "loc " << i << " is bitcast, recursing\n";
                }
                AliasResult bc_base_aliases = alias(locs[i^1], Location(bc_base, locs[i].Size));
                if (VERBOSITY("opt.aa") >= 2) {
                    indent(); bc_base->dump();
                    indent(); errs() << "bc base aliases: " << bc_base_aliases << '\n';
                }
...

            {
                ...
                if (GIa && GIb) {
                    ...
                    if (VERBOSITY("opt.aa") >= 2) {
                        indent(); errs() << "2 geps, recursing\n";
                    }
                    AliasResult bases_alias = alias(Location(baseA), Location(baseB));
                    if (VERBOSITY("opt.aa") >= 2) {
                        indent(); errs() << "2gep base aliases: " << bases_alias << '\n';
                        indent(); LocA.Ptr->dump();
                        indent(); LocB.Ptr->dump();
                    }
                    ...
                    if (bases_alias == MustAlias) {
                        ...
                        if (accumA && accumB) {
                            if (VERBOSITY("opt.aa") >= 2) {
                                indent(); errs() << offsetA << ' ' << LocA.Size << ' ' << offsetB << ' ' << LocB.Size << '\n';
                            }
                            int sizeA = LocA.Size;
                            int sizeB = LocB.Size;
                            if (offsetA == offsetB) {
...
INITIALIZE_AG_PASS(PystonAA, AliasAnalysis, "pystonaa",
        "Pyston AA",
        false, true, false)
