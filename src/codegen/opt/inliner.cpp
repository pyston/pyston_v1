    ...
    public:
        ...
        virtual llvm::Value* materializeValueFor(llvm::Value* v) {
            ...
            if (llvm::Function *f = llvm::dyn_cast<llvm::Function>(v)) {
                ...
                r = addressee;
                //r = new llvm::GlobalAlias(alias->getType(), alias->getLinkage(), alias->getName(), addressee, new_module);
        ...
        bool _runOnFunction(llvm::Function &f) {
            ...
            for (int passnum = 0; passnum < NPASSES; passnum++) {
                ...
                while (calls.size()) {
                    ...
                    if (IC.isAlways()) {
                        if (VERBOSITY("irgen.inlining") >= 2) llvm::errs() << "always inline\n";
                        do_inline = true;
                    } else if (IC.isNever()) {
                        if (VERBOSITY("irgen.inlining") >= 2) llvm::errs() << "never inline\n";
                        do_inline = false;
                    } else {
                        if (VERBOSITY("irgen.inlining") >= 2) llvm::errs() << "Inline cost: " << IC.getCost() << '\n';
                        do_inline = (bool)IC;
                    }
